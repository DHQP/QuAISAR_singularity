...
if [[ -s "${output_dir}/time_summary.txt" ]]; then
	mv "${output_dir}/time_summary.txt" "${output_dir}/${1}_time_summary.txt"
fi
printf "%-20s: %-8s : %s\\n" "Summarized" "SUCCESS" "${today}"
if [[ -s "${output_dir}/${1}_time_summary.txt" ]]; then
	time=$(tail -1 "${output_dir}/${1}_time_summary.txt" | cut -d' ' -f3)
...
if [[ -s "${output_dir}/FASTQs/${1}_R1_001.fastq" ]] && [[ -s "${output_dir}/FASTQs/${1}_R2_001.fastq" ]]; then
	printf "%-20s: %-8s : %s\\n" "FASTQs" "SUCCESS" "Unzipped"
	:
elif [[ -s "${output_dir}/FASTQs/${1}_R1_001.fastq" ]]; then
	printf "%-20s: %-8s : %s\\n" "FASTQs" "WARNING" "Only R1 found"
	if [ "${status}" = "SUCCESS" ] || [ "${status}" = "ALERT" ]; then
		status="WARNING"
	fi
elif [[ -s "${output_dir}/FASTQs/${1}_R2_001.fastq" ]]; then
	printf "%-20s: %-8s : %s\\n" "FASTQs" "WARNING" "Only R2 found"
	if [ "${status}" = "SUCCESS" ] || [ "${status}" = "ALERT" ]; then
		status="WARNING"
	fi
elif [[ -s "${output_dir}/FASTQs/${1}_R1_001.fastq.gz" ]] && [[ -s "${output_dir}/FASTQs/${1}_R2_001.fastq.gz" ]]; then
...
#Checking QC counts
if [[ -s "${output_dir}/preQCcounts/${1}_counts.txt" ]]; then
	reads_pre=$(head -n 1 "${output_dir}/preQCcounts/${1}_counts.txt" | cut -d'	' -f13)
	pairs_pre=$((reads_pre/2))
	Q30_R1=$(head -n 1 "${output_dir}/preQCcounts/${1}_counts.txt" | cut -d'	' -f10)
	Q30_R1_rounded=$(echo "${Q30_R1}"  | cut -d'.' -f2)
	Q30_R1_rounded=$(echo "${Q30_R1_rounded::2}")
	Q30_R2=$(head -n 1 "${output_dir}/preQCcounts/${1}_counts.txt" | cut -d'	' -f11)
...
#if [[ -s "${output_dir}/removedAdapters/${1}-noPhiX-R1.fsq" ]] && [[ -s "${output_dir}/removedAdapters/${1}-noPhiX-R2.fsq" ]]; then
#	#printf "%-20s: %-8s : %s\\n" "Adapter Removal" "SUCCESS" "Found"
#	:
#elif [[ -s "${output_dir}/removedAdapters/${1}-noPhiX-R1.fsq" ]]; then
#	printf "%-20s: %-8s : %s\\n" "Adapter Removal" "WARNING" "Only R1 found"
#	if [ "${status}" = "SUCCESS" ] || [ "${status}" = "ALERT" ]; then
#		status="WARNING"
#	fi
#elif [[ -s "${output_dir}/removedAdapters/${1}-noPhiX-R2.fsq" ]]; then
...
if [[ -s "${output_dir}/trimmed/${1}_R1_001.paired.fq" ]] && [[ -s "${output_dir}/trimmed/${1}_R2_001.paired.fq" ]]; then
	printf "%-20s: %-8s : %s\\n" "Trimming" "SUCCESS" "Unzipped"
	:
elif [[ -s "${output_dir}/trimmed/${1}_R1_001.paired.fq" ]]; then
	printf "%-20s: %-8s : %s\\n" "Trimming" "WARNING" "Only R1 found"
	if [ "${status}" = "SUCCESS" ] || [ "${status}" = "ALERT" ]; then
		status="WARNING"
	fi
elif [[ -s "${output_dir}/trimmed/${1}_R2_001.paired.fq" ]]; then
	printf "%-20s: %-8s : %s\\n" "Trimming" "WARNING" "Only R2 found"
	if [ "${status}" = "SUCCESS" ] || [ "${status}" = "ALERT" ]; then
		status="WARNING"
	fi
elif [[ -s "${output_dir}/trimmed/${1}_R1_001.paired.fq.gz" ]] && [[ -s "${output_dir}/trimmed/${1}_R2_001.paired.fq.gz" ]]; then
	printf "%-20s: %-8s : %s\\n" "Trimming" "SUCCESS" "Zipped"
	:
elif [[ -s "${output_dir}/trimmed/${1}_R1_001.paired.fq.gz" ]]; then
	printf "%-20s: %-8s : %s\\n" "Trimming" "WARNING" "Zipped, but only R1 found"
	if [ "${status}" = "SUCCESS" ] || [ "${status}" = "ALERT" ]; then
		status="WARNING"
	fi
elif [[ -s "${output_dir}/trimmed/${1}_R2_001.paired.fq.gz" ]]; then
...
if [[ -s "${output_dir}/preQCcounts/${1}_trimmed_counts.txt" ]]; then
	reads_post=$(head -n 1 "${output_dir}/preQCcounts/${1}_trimmed_counts.txt" | cut -d'	' -f13)
...
if [[ -s "${output_dir}/kraken/preAssembly/${1}_paired.kraken" ]]; then
...
if [[ "${kraken_pre_success}" = true ]]; then
	if [[ -s "${output_dir}/kraken/preAssembly/${1}_paired.krona" ]] && [[ -s "${output_dir}/kraken/preAssembly/${1}_paired.html" ]]; then
...
if [[ -s "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" ]]; then
	# Extracts many elements of the summary file to report unclassified and species classified reads and percentages
	unclass=$(head -n 1 "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" | cut -d' ' -f2)
	#true_unclass=$(head -n 1 "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
	domain=$(sed -n '2p' "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" | cut -d' ' -f2)
	genuspre=$(sed -n '7p' "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" | cut -d' ' -f4)
	speciespre=$(sed -n '8p' "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" | cut -d' ' -f4)
	speciespercent=$(sed -n '8p' "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" | cut -d' ' -f2)
	#true_speciespercent=$(sed -n '8p' "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
...
# If no summary file was found
	...
	printf "%-20s: %-8s : %s\\n" "Pre Classify" "FAILED" "${output_dir}/kraken/preAssembly/${1}_kraken_summary_paired.txt not found"
	status="FAILED"
fi

# Quick separate check for contamination by finding # of species above ${contamination_threshold} in list file from kraken
if [[ -s "${output_dir}/kraken/preAssembly/${1}_paired.list" ]]; then
	number_of_species=0
	...
	done < ${output_dir}/kraken/preAssembly/${1}_paired.list
...
if [[ -s "${output_dir}/gottcha/gottcha_S/${1}.gottcha_full.tsv" ]] && [[ -s "${output_dir}/gottcha/${1}_species.krona.html" ]]; then
	#printf "%-20s: %-8s : %s\\n" "GOTTCHA_S" "SUCCESS" "Found"
	:
elif [[ -s "${output_dir}/gottcha/gottcha_S/${1}.gottcha_full.tsv" ]]; then
	printf "%-20s: %-8s : %s\\n" "GOTTCHA_S" "WARNING" "No Krona output found"
	if [ "${status}" = "SUCCESS" ] || [ "${status}" = "ALERT" ]; then
		status="WARNING"
	fi
elif [[ -s "${output_dir}/gottcha/${1}_species.krona.html" ]]; then
...
if [[ -s "${output_dir}/gottcha/${1}_gottcha_species_summary.txt" ]]; then
	# Extracts many elements of the summary file to report unclassified and species classified reads and percentages
	unclass=$(head -n 1 "${output_dir}/gottcha/${1}_gottcha_species_summary.txt" | cut -d' ' -f2)
	#true_unclass=$(head -n 1 "${output_dir}/gottcha/${1}_gottcha_species_summary.txt" | cut -d' ' -f3) # | sed -r 's/[)]+/%)/g')
	phylumpercent=$(sed -n '3p' "${output_dir}/gottcha/${1}_gottcha_species_summary.txt" | cut -d' ' -f2)
	genuspre=$(sed -n '7p' "${output_dir}/gottcha/${1}_gottcha_species_summary.txt"| cut -d' ' -f4)
	speciespre=$(sed -n '8p' "${output_dir}/gottcha/${1}_gottcha_species_summary.txt" | cut -d' ' -f5)
	speciespercent=$(sed -n '8p' "${output_dir}/gottcha/${1}_gottcha_species_summary.txt" | cut -d' ' -f2)
	true_speciespercent=$(sed -n '8p' "${output_dir}/gottcha/${1}_gottcha_species_summary.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
...
# If the summary file does not exist, report as such
	...
	printf "%-20s: %-8s : %s\\n" "GottchaV1 Classifier" "FAILED" "${output_dir}/gottcha/${1}_gottcha_species_summary.txt not found"
	status="FAILED"
fi

# Quick separate check for contamination by finding # of species above ${contamination_threshold} in list file from kraken
if [[ -s "${output_dir}/gottcha/gottcha_S/${1}.gottcha.tsv" ]]; then
	number_of_species=0
	while IFS= read -r line; do
	...
	done < ${output_dir}/gottcha/gottcha_S/${1}.gottcha.tsv
	if [[ $number_of_species -gt 1 ]]; then
...
if [[ -s "${output_dir}/Assembly/scaffolds.fasta" ]]; then
	# Count the number of '>' in the assembly file before trimming
	full_scaffolds=">"
	full_scaffolds=$(grep -c ${full_scaffolds} "${output_dir}/Assembly/scaffolds.fasta")
...
if [[ -d "${output_dir}/plasFlow" ]]; then
	if [[ -s "${output_dir}/plasFlow/Unicycler_assemblies/${1}_uni_assembly/${1}_plasmid_assembly.fasta" ]]; then
		# Count the number of '>' in the assembly file before trimming
		plas_scaffolds=">"
		plas_scaffolds=$(grep -c ${plas_scaffolds} "${output_dir}/plasFlow/Unicycler_assemblies/${1}_uni_assembly/${1}_plasmid_assembly.fasta")
...
if [[ -s "${output_dir}/Assembly/${1}_scaffolds_trimmed.fasta" ]]; then
	# Count the number of '>' still remaining after trimming the contig file
	full_longies=">"
	full_longies=$(grep -c ${full_longies} "${output_dir}/Assembly/${1}_scaffolds_trimmed.fasta")
...
if [[ "${plasmidsFoundviaplasFlow}" -eq 1 ]]; then
	if [[ -s "${output_dir}/plasFlow/Unicycler_assemblies/${1}_uni_assembly/${1}_plasmid_assembly_trimmed.fasta" ]]; then
		# Count the number of '>' still remaining after trimming the contig file
		plas_longies=">"
		plas_longies=$(grep -c ${plas_longies} "${output_dir}/plasFlow/Unicycler_assemblies/${1}_uni_assembly/${1}_plasmid_assembly_trimmed.fasta")
		# Calculate the number of lost (short) scaffolds
	...
	elif [[ -f "${output_dir}/plasFlow/Unicycler_assemblies/${1}_uni_assembly/${1}_plasmid_assembly_trimmed.fasta" ]]; then
...
if [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled.kraken" ]]; then
...
if [[ "${kraken_post_success}" = true ]]; then
	if [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled.krona" ]] && [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled.html" ]]; then
...
if [[ -s "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" ]]; then
	# Extracts many elements of the summary file to report unclassified and species classified reads and percentages
	unclass=$(head -n 1 "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" | cut -d' ' -f2)
	#true_unclass=$(head -n 1 "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
	domain=$(sed -n '2p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" | cut -d' ' -f2)
	genuspost=$(sed -n '7p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" | cut -d' ' -f4)
	speciespost=$(sed -n '8p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" | cut -d' ' -f4)
	speciespercent=$(sed -n '8p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" | cut -d' ' -f2)
	#true_speciespercent=$(sed -n '8p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
	# If there are no reads at the domain level, then report no classified reads
	if (( $(echo "${domain} <= 0" | bc -l) )); then
...
if [[ ! -s "${output_dir}/kraken/postAssembly/${1}_assembled_BP.kraken" ]]; then
	if [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled.kraken" ]]; then
...
# Quick separate check for contamination by finding # of species above ${contamination_threshold} in list file from kraken
if [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled.list" ]]; then
	number_of_species=0
	while IFS= read -r line; do
		arrLine=(${line})
		# First element in array is the percent of reads identified as the current taxa
	...
	done < ${output_dir}/kraken/postAssembly/${1}_assembled.list
	if [[ $number_of_species -gt 1 ]]; then
...
if [[ "${kraken_weighted_success}" = true ]]; then
	if [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled_weighted.krona" ]] && [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled_weighted_BP_krona.html" ]]; then
...
if [[ -s "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" ]]; then
	# Extracts many elements of the summary file to report unclassified and species classified reads and percentages
	unclass=$(head -n 1 "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" | cut -d' ' -f2)
	#true_unclass=$(head -n 1 "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
	domain=$(sed -n '2p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" | cut -d' ' -f2)
	genusweighted=$(sed -n '7p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" | cut -d' ' -f4)
	speciesweighted=$(sed -n '8p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" | cut -d' ' -f4)
	speciespercent=$(sed -n '8p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" | cut -d' ' -f2)
	#true_speciespercent=$(sed -n '8p' "${output_dir}/kraken/postAssembly/${1}_kraken_summary_assembled_BP.txt" | cut -d' ' -f3 | sed -r 's/[)]+/%)/g')
	# If there are no reads at the domain level, then report no classified reads
	if (( $(echo "${domain} <= 0" | bc -l) )); then
...
# Quick separate check for contamination by finding # of species above ${contamination_threshold} in list file from kraken
if [[ -s "${output_dir}/kraken/postAssembly/${1}_assembled_BP.list" ]]; then
	number_of_species=0
	while IFS= read -r line; do
		arrLine=(${line})
		# First element in array is the percent of reads identified as the current taxa
	...
	done < ${output_dir}/kraken/postAssembly/${1}_assembled_BP.list
	if [[ $number_of_species -gt 1 ]]; then
...
#Check QUAST
if [[ -s "${output_dir}/Assembly_Stats/${1}_report.tsv" ]]; then
	# Extract the useful bits and report (to compare to Toms)
	contig_num=$(sed -n '14p' "${output_dir}/Assembly_Stats/${1}_report.tsv"| sed -r 's/[\t]+/ /g' | cut -d' ' -f3 )
	ass_length=$(sed -n '16p' "${output_dir}/Assembly_Stats/${1}_report.tsv" | sed -r 's/[\t]+/ /g' | cut -d' ' -f3)
	N50=$(sed -n '18p' "${output_dir}/Assembly_Stats/${1}_report.tsv"  | sed -r 's/[\t]+/ /g'| cut -d' ' -f2)
	GC_con=$(sed -n '17p' "${output_dir}/Assembly_Stats/${1}_report.tsv" | sed -r 's/[\t]+/ /g' | cut -d' ' -f3)
...
if [[ "${plasmidsFoundviaplasFlow}" -eq 1 ]]; then
	if [[ -s "${output_dir}/Assembly_Stats_plasFlow/${1}_report.tsv" ]]; then
		# Extract the useful bits and report (to compare to Toms)
		contig_num_plas=$(sed -n '14p' "${output_dir}/Assembly_Stats_plasFlow/${1}_report.tsv"| sed -r 's/[\t]+/ /g' | cut -d' ' -f3 )
		ass_length_plas=$(sed -n '16p' "${output_dir}/Assembly_Stats_plasFlow/${1}_report.tsv" | sed -r 's/[\t]+/ /g' | cut -d' ' -f3)
		N50_plas=$(sed -n '18p' "${output_dir}/Assembly_Stats_plasFlow/${1}_report.tsv"  | sed -r 's/[\t]+/ /g'| cut -d' ' -f2)
		GC_con_plas=$(sed -n '17p' "${output_dir}/Assembly_Stats_plasFlow/${1}_report.tsv" | sed -r 's/[\t]+/ /g' | cut -d' ' -f3)
...
if [[ ! -s "${output_dir}/${1}.tax" ]]; then
	"${shareScript}/determine_taxID.sh" "${1}" "${2}"
fi

source_call=$(head -n1 "${output_dir}/${1}.tax")
while IFS= read -r line; do
...
# check coverage
if [[ -s "${output_dir}/preQCcounts/${1}_counts.txt" ]]; then
	line=$(head -n 1 "${output_dir}/preQCcounts/${1}_counts.txt")
...
if [[ -s "${output_dir}/preQCcounts/${1}_trimmed_counts.txt" ]]; then
	line=$(head -n 1 "${output_dir}/preQCcounts/${1}_trimmed_counts.txt")
	IFS='	' read -r -a qcs <<< "${line}"
	read_qc_info=${qcs[@]:1}
	# Extract q30 reads from qcCounts to calculate average coverage as q30_reads/assembly_length
	q30_reads=$(echo "${read_qc_info}" | awk -F ' ' '{print $2}')
...
# Check prokka
if [[ -s "${output_dir}/prokka/${1}_PROKKA.gbf" ]]; then
	# Counts the number of genes present in the file using the 'CDS' identifier
	genes="CDS"
	genes=$(grep -c ${genes} "${output_dir}/prokka/${1}_PROKKA.gbf")
	printf "%-20s: %-8s : %s\\n" "prokka" "SUCCESS" "${genes} genes found"
elif [[ -s "${output_dir}/prokka/${1}_PROKKA.gbk" ]]; then
	# Counts the number of genes present in the file using the 'CDS' identifier
	genes="CDS"
	genes=$(grep -c ${genes} "${output_dir}/prokka/${1}_PROKKA.gbk")
	printf "%-20s: %-8s : %s\\n" "prokka" "SUCCESS" "${genes} genes found"
else
...
#Check BUSCO
if [[ -s "${output_dir}/BUSCO/short_summary_${1}.txt" ]]; then
	...
	done < "${output_dir}/BUSCO/short_summary_${1}.txt"
...
if [[ -s "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${dec_genus,}).txt" ]]; then
	mv "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${dec_genus,}).txt" "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${dec_genus^}).txt"
		 #"${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${dec_genus^}).txt"
fi
if [[ -f "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_All).txt" ]]; then
	#echo "ALL"
	ani_info=$(head -n 1 "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_All).txt")
	ani_found=true
elif [[ -f "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${dec_genus^}).txt" ]]; then
	ani_info=$(head -n 1 "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${dec_genus^}).txt")
	ani_found=true
else
	for file in "${output_dir}/ANI/"*
	do
...
if [[ -f "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${REFSEQ_date}).txt" ]]; then
	#echo "ALL"
	ani_info=$(head -n 1 "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_${REFSEQ_date}).txt")
	percent_match=$(echo "${ani_info}" | cut -d'.' -f1)
	coverage_match=$(echo "${ani_info}" | cut -d'-' -f2 | cut -d'.' -f1)
	#echo "${percent_match--}"
	if [[ "${percent_match}" = "0." ]]; then
	...
	if [[ -f "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_REFSEQ*).txt" ]]; then
		old_ani_file=$(find ${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_REFSEQ*).txt -maxdepth 1 -type f -printf '%p\n' | sort -k2,2 -rt '_' -n)
...
#Check ANI of OSII
if [[ -f "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_OSII).txt" ]]; then
	#echo "ALL"
	ani_info=$(head -n 1 "${output_dir}/ANI/best_ANI_hits_ordered(${1}_vs_OSII).txt")
	percent_match=$(echo "${ani_info}" | cut -d'.' -f1)
	coverage_match=$(echo "${ani_info}" | cut -d'-' -f2 | cut -d'.' -f1)
	#echo "${percent_match--}"
	if [[ "${percent_match}" = "0." ]]; then
...
if [[ -d "${output_dir}/c-sstar/" ]]; then
	...
	csstar_file=$(find ${output_dir}/c-sstar/${1}.ResGANNCBI*.${gapping}_${sim}_sstar_summary.txt -maxdepth 1 -type f -printf '%p\n' | sort -k2,2 -rt '_' -n | head -n 1)
...
if [[ "${plasmidsFoundviaplasFlow}" -eq 1 ]]; then
	#Check c-SSTAR
	if [[ -d "${output_dir}/c-sstar_plasFlow/" ]]; then
		if [[ ! -z "${3}" ]]; then
			gapping="${3}"
		else
			gapping="gapped"
		...
		csstar_plasFlow_file=$(find ${output_dir}/c-sstar_plasFlow/${1}.ResGANNCBI*.${gapping}_${sim}_sstar_summary.txt -maxdepth 1 -type f -printf '%p\n' | sort -k2,2 -rt '_' -n | head -n 1)
		if [[ -z "${csstar_plasFlow_file}" ]]; then
			printf "%-20s: %-8s : %s\\n" "c-SSTAR_plasFlow" "FAILED" "/c-sstar_plasFlow/ does not have an sstar_summary file"
			echo "Looking for ${output_dir}/c-sstar_plasFlow/${1}.ResGANNCBI.${gapping}_${sim}_sstar_summary.txt"
			status="FAILED"
		else
...
#Check c-SSTAR
if [[ -d "${output_dir}/GAMA/" ]]; then
	GAMA_file=$(find ${output_dir}/GAMA -maxdepth 1 -type f -name "${1}.ResGANNCBI*.GAMA"   -printf '%p\n' | sort -k2,2 -rt '_' -n | head -n 1)
	if [[ -z "${GAMA_file}" ]]; then
...
# #Check c-SSTAR on plasmid Assembly
if [[ "${plasmidsFoundviaplasFlow}" -eq 1 ]]; then
	if [[ -d  "${output_dir}/GAMA_plasFlow" ]]; then
		#Check c-SSTAR
		GAMA_plasFlow_file=$(find ${output_dir}/GAMA_plasFlow -maxdepth 1 -type f -name "${1}.ResGANNCBI*.GAMA"   -printf '%p\n' | sort -k2,2 -rt '_' -n | head -n 1)
...
# check SRST2 output
if [[ -d "${output_dir}/srst2/" ]]; then
	ResGANNCBI_srst2_file=$(find ${output_dir}/srst2/${1}__genes__ResGANNCBI*_srst2__results.txt -maxdepth 1 -type f -printf '%p\n' | sort -k6,6 -rt '_' -n | head -n 1)
...
# check MLST
if [[ -d "${output_dir}/MLST/" ]]; then
	if [[ -s "${output_dir}/MLST/${1}_Pasteur.mlst" ]] || [[ -s "${output_dir}/MLST/${1}.mlst" ]]; then
		if [[ -f "${output_dir}/MLST/${1}.mlst" ]]; then
			mv "${output_dir}/MLST/${1}.mlst" "${output_dir}/MLST/${1}_Pasteur.mlst"
		fi
		if [[ -f "${output_dir}/MLST/${1}_ecoli_2.mlst" ]]; then
			mv "${output_dir}/MLST/${1}_Pasteur.mlst" "${output_dir}/MLST/${1}_Achtman.mlst"
			mv "${output_dir}/MLST/${1}_ecoli_2.mlst" "${output_dir}/MLST/${1}_Pasteur.mlst"
		fi
		info=$(head -n 1 "${output_dir}/MLST/${1}_Pasteur.mlst")
	...
	if [[ "${dec_genus}" = "Acinetobacter" ]]; then
		if [[ -s "${output_dir}/MLST/${1}_abaumannii.mlst" ]] || [[ -s "${output_dir}/MLST/${1}_Oxford.mlst" ]]; then
			if [[ -s "${output_dir}/MLST/${1}_abaumannii.mlst" ]]; then
				mv "${output_dir}/MLST/${1}_abaumannii.mlst" "${output_dir}/MLST/${1}_Oxford.mlst"
			fi
			info=$(tail -n 1 "${output_dir}/MLST/${1}_Oxford.mlst")
			mlstype=$(echo "${info}" | cut -d'	' -f3)
			mlstdb=$(echo "${info}" | cut -d'	' -f2)
			#echo "'${mlstdb}:${mlstype}'"
	...
	if [[ "${dec_genus}" = "Escherichia" ]]; then
		if [[ -s "${output_dir}/MLST/${1}_Achtman.mlst" ]]; then
			info=$(tail -n 1 "${output_dir}/MLST/${1}_Achtman.mlst")
			mlstype=$(echo "${info}" | cut -d'	' -f3)
			mlstdb=$(echo "${info}" | cut -d'	' -f2)
			#echo "'${mlstdb}:${mlstype}'"
	...
	# Check srst2 MLSTs
	num_srst2_mlsts=$(find ${output_dir}/MLST -type f -name "*_srst2_*.mlst" | wc -l)
	...
	if [[ "${num_srst2_mlsts}" -eq 0 ]]; then
		...
		srst_mlst=$(find ${output_dir}/MLST -type f -name "*_srst2_*.mlst")
		...
		if [[ "${dec_genus}" = "Acinetobacter" ]]; then
			if [[ -f "${output_dir}/MLST/${1}_srst2_Acinetobacter_baumannii#1-Oxford.mlst" ]]; then
				srst_mlst="${output_dir}/MLST/${1}_srst2_Acinetobacter_baumannii#1-Oxford.mlst"
				mlstype=$(tail -n1 ${srst_mlst} | cut -d'	' -f2)
			...
			if [[ -f "${output_dir}/MLST/${1}_srst2_Acinetobacter_baumannii#2-Pasteur.mlst" ]]; then
				srst_mlst="${output_dir}/MLST/${1}_srst2_Acinetobacter_baumannii#2-Pasteur.mlst"
				mlstype=$(tail -n1 ${srst_mlst} | cut -d'	' -f2)
			...
			if [[ -f "${output_dir}/MLST/${1}_srst2_Escherichia_coli#1-Achtman.mlst" ]]; then
				srst_mlst="${output_dir}/MLST/${1}_srst2_Escherichia_coli#1-Achtman.mlst"
				mlstype=$(tail -n1 ${srst_mlst} | cut -d'	' -f2)
			...
			if [[ -f "${output_dir}/MLST/${1}_srst2_Escherichia_coli#2-Pasteur.mlst" ]]; then
				srst_mlst="${output_dir}/MLST/${1}_srst2_Escherichia_coli#2-Pasteur.mlst"
				mlstype=$(tail -n1 ${srst_mlst} | cut -d'	' -f2)
...
if [[ -d "${output_dir}/16s/" ]]; then
	if [[ -s "${output_dir}/16s/${1}_16s_blast_id.txt" ]]; then
		info_b=$(head -n 1 "${output_dir}/16s/${1}_16s_blast_id.txt")
		...
		info_l=$(tail -n 1 "${output_dir}/16s/${1}_16s_blast_id.txt")
...
# check plasmids
if [[ -d "${output_dir}/plasmidFinder/" ]]; then
	if [[ -s "${output_dir}/plasmidFinder/${1}_results_table_summary.txt" ]]; then
		...
		done < "${output_dir}/plasmidFinder/${1}_results_table_summary.txt"
...
if [[ "plasmidsFoundviaplasFlow" -eq 1 ]]; then
	if [[ -d "${output_dir}/plasmidFinder_on_plasFlow/" ]]; then
		if [[ -s "${output_dir}/plasmidFinder_on_plasFlow/${1}_results_table_summary.txt" ]]; then
			number_of_plasmids=0
			while read line_in; do
				line_in=$(echo ${line_in} | cut -d' ' -f1)
				if [[ "${line_in}" = "No" ]] || [[ "${line_in}" = "Enterococcus,Streptococcus,Staphylococcus" ]] || [[ "${line_in}" = "Enterobacteriaceae" ]] || [[ "${line_in}" = "Plasmid" ]]; then
